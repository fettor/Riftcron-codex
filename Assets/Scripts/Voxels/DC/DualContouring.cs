...
namespace Tuntenfisch.Voxels.DC
{
    ...
    public class DualContouring : MonoBehaviour
    {
        ...
        private class Worker : IDisposable
        {
            ...
            {
                // Create CPU buffers.
                int maxNumberOfVertices = math.max(1, m_parent.m_voxelConfig.VoxelVolumeConfig.CellCount);
                ...
                int maxNumberOfTriangles = math.max(1, 3 * 6 * (int)math.round(math.pow(m_parent.m_voxelConfig.VoxelVolumeConfig.NumberOfCellsAlongAxis - 1, 3)));
                ...
                if (m_generatedVerticesBuffer0?.Count != m_generatedVertices.Length)
                {
                    ...
                    m_generatedVerticesBuffer0 = new AsyncComputeBuffer(math.max(1, m_generatedVertices.Length), GPUVertex.SizeInBytes, ComputeBufferType.Counter);
                }
                ...
                if (m_generatedVerticesBuffer1?.Count != m_generatedVertices.Length)
                {
                    ...
                    // The counter attached to this compute buffer stores the number of vertices generated by dual contouring.
                    m_generatedVerticesBuffer1 = new AsyncComputeBuffer(math.max(1, m_generatedVertices.Length), GPUVertex.SizeInBytes, ComputeBufferType.Counter);
                }
                ...
                if (m_generatedTrianglesBuffer?.Count != m_generatedTriangles.Length)
                {
                    ...
                    m_generatedTrianglesBuffer = new AsyncComputeBuffer(math.max(1, m_generatedTriangles.Length), sizeof(uint), ComputeBufferType.Raw);
                }
            }
